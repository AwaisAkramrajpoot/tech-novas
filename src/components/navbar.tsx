...
export function Navbar() {
  ...
  const services = [
    ...
    { name: "Application Maintenance & Support", href: "/services/maintenance-support" },
  ]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link href="/" className="flex items-center gap-2 group animate-fade-in-down">
            <div className="w-8 h-8 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-lg flex items-center justify-center font-bold text-white text-lg group-hover:shadow-lg group-hover:shadow-cyan-500/50 transition-all duration-300">
              N
            </div>
            <span className="text-xl font-bold text-navy hover:text-cyan-600 transition-colors">Novaa Tech</span>
          </Link>
          ...
          <div className="hidden md:flex gap-8">
            ...
            >
              <button className="flex items-center gap-1 text-gray-700 hover:text-cyan-600 transition-colors font-medium group">
                Services
              ...
              {/* Dropdown Menu */}
              {isServicesOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-2 animate-fade-in-up z-50">
                  {services.map((service, i) => (
                    <Link
                      key={service.href}
                      href={service.href}
                      className="block px-4 py-2 text-gray-700 hover:text-cyan-600 hover:bg-gray-50 transition-colors font-medium"
            ...
            <Link href="/projects" className="text-gray-700 hover:text-cyan-600 transition-colors font-medium">
              Projects
            </Link>
                ...
                <div className="pl-4 space-y-1 mt-2 animate-fade-in-up">
                  {services.map((service) => (
                    <Link
                      key={service.href}
                      href={service.href}
                      className="block px-4 py-2 text-gray-700 hover:text-white hover:bg-cyan-600 rounded-lg transition-all duration-300"
                      onClick={() => {
                        setIsOpen(false)
